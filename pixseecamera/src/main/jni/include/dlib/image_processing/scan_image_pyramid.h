        ...
        for (unsigned long l = 0; l < feats.size(); ++l) {
            ...
            for (unsigned long i = 0; i < det_templates.size(); ++i) {
                ...
                for (unsigned long j = 0; j < stationary_region_rects.size(); ++j) {
                    stationary_region_rects[j] = std::make_pair(j, translate_rect(feats[l].image_to_feat_space(det_templates[i].rects[j]),
                                                                                  offset));
                }
...
// ----------------------------------------------------------------------------------------
    ...
    get_full_object_detection(
        ...
        for (unsigned long i = 0; i < get_num_movable_components_per_detection_template(); ++i) {
            ...
            const unsigned long offset = get_num_detection_templates() +
                                         feats_config.get_num_dimensions() * (i + get_num_stationary_components_per_detection_template());
