    ...
    std::string http_client::urldecode(const std::string &s) {
        ...
        for (char const *p_read = s.c_str(), *p_end = (s.c_str() + s.size()); p_read < p_end; p_read++) {
            if (p_read[0] == '%' && p_read + 1 != p_end && p_read + 2 != p_end && isXdigit(p_read[1]) && isXdigit(p_read[2])) {
                ss << static_cast<char>((((p_read[1] & 0xf) + ((p_read[1] >= 'A') ? 9 : 0)) << 4) |
...
// POST
    const std::string &http_client::post_url(const std::string &url, const string_to_stringmap &postvars,
                                             const string_to_stringmap &filenames) {
        std::string CT;
...
// ----------------------------------------------------------------------------------------
    ...
    std::string http_client::build_post(std::string &content_type, const string_to_stringmap &postvars,
                                        const string_to_stringmap &filenames_in) const {
...
// ----------------------------------------------------------------------------------------
    ...
    bool http_client::grab_url(const std::string &url, const std::string &method, const std::string &post_body) {
        ...
        for (stringmap::iterator ci = cookies.begin(); ci != cookies.end(); ++ci) {
            std::string var = ci->first;
            trim(var);
            std::string val = ci->second;
            trim(val);
        ...
        std::stringstream ss;
        {
                    ...
                    while (true) {
                        ...
                        if (BR_CASECMP(header_name.c_str(), "Content-Length", 14) == 0) {
                            ...
                            for (cur_pos = 0; cur_pos < header_value.size(); cur_pos++) {
                                ...
                                pos_is = work.find('=');
                                if (pos_is != std::string::npos) { // Hmmm? what in the else case?
            ...
            switch (bytes_read) {
                case dlib::TIMEOUT:
                    ...
                    return false;
                    break;
                case dlib::WOULDBLOCK:
                    error_field = "Would block";
                    return false;
                    break;
                case dlib::OTHER_ERROR:
                    error_field = "Other error";
                    return false;
                    break;
                case dlib::SHUTDOWN:
                    error_field = "Timeout";
