...
// ----------------------------------------------------------------------------------------
    ...
    template<typename T>
    unichar surrogate_pair_to_unichar(T first, T second) {
...
// ----------------------------------------------------------------------------------------
    ...
    template<int N>
    void wstr2ustring_t(const wchar_t *src, size_t src_len, ustring &dest);

    template<>
    void wstr2ustring_t<4>(const wchar_t *src, size_t, ustring &dest) {
        dest.assign((const unichar *) (src));
    }

    template<>
    void wstr2ustring_t<2>(const wchar_t *src, size_t src_len, ustring &dest) {
        size_t wlen = 0;
