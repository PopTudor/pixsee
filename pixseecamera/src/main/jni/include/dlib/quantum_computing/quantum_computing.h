        ...
        qc_scalar_type compute_state_element(
            ...
            for (long c = 0; c < T::dims; ++c) {
                if (lhs(row_idx / U::dims, c) != qc_scalar_type(0)) {
                    result +=
                            lhs(row_idx / U::dims, c) * rhs.compute_state_element(subm(reg, c * U::dims, 0, U::dims, 1), row_idx % U::dims);
                }
            }
...
// ----------------------------------------------------------------------------------------
    ...
    namespace qc_helpers {
        ...
        struct combine_gates<T, U, V, typename enable_if_c<(is_composite_gate<U>::value == true)>::type> {
            ...
            static const result_type eval(
                ...
                return composite_gate<T, inner_type>(lhs.lhs,
                                                     combine_gates<typename U::lhs_type, typename U::rhs_type, V>::eval(lhs.rhs, rhs));
            }
    ...
    template<typename T>
    void operator,(const gate_exp<T> &, int) { COMPILE_TIME_ASSERT(sizeof(T) > 100000000); }

    template<typename T>
    void operator,(int, const gate_exp<T> &) { COMPILE_TIME_ASSERT(sizeof(T) > 100000000); }
...
// ----------------------------------------------------------------------------------------
    ...
    namespace quantum_gates {
        ...
        public:
            ...
            COMPILE_TIME_ASSERT(
                    (control_bit1 < target_bit && control_bit2 < target_bit) || (control_bit1 > target_bit && control_bit2 > target_bit));
