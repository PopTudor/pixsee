                ...
                for (; i < w.row_filters.size(); ++i) {
                    for (unsigned long j = 0; j < w.row_filters[i].size(); ++j) {
                        if (saliency_image.size() == 0)
                            area = float_spatially_filter_image_separable(feats[i], saliency_image, w.row_filters[i][j],
...
// ----------------------------------------------------------------------------------------
    ...
    get_mapped_rect_and_metadata(
        ...
        for (unsigned long l = 0; l < number_pyramid_levels; ++l) {
            ...
            const rectangle win_image_space = pyr.rect_up(
                    fe.feats_to_image(centered_rect(center(rect_fhog_space), width - 2 * padding, height - 2 * padding), cell_size, height,
                                      width), l);
...
// ----------------------------------------------------------------------------------------
    ...
    matrix<unsigned char> draw_fhog(
        ...
        typename scan_fhog_pyramid<Pyramid_type, feature_extractor_type>::fhog_filterbank fb = detector.get_scanner().build_fhog_filterbank(
