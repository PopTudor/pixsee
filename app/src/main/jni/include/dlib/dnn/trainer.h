    ...
    private:
            ...
            while (job_pipe.dequeue(next_job)) {
                ...
                for (size_t i = 0; i < devices.size(); ++i)
                    tp[i]->add_task_by_value(
                            [&, i](double &loss) { loss = compute_parameter_gradients(i, next_job, pick_which_run_update); }, losses[i]);
        ...
        friend void deserialize(dnn_trainer &item, std::istream &in) {
            ...
            if (num_layers != dnn_trainer::num_layers) {
                ...
                sout << "a different number of layers.  We expected the number of layers to be " << dnn_trainer::num_layers << " but" <<
                std::endl;
