package com.marked.pixsee.fragment.chat
...
import com.marked.pixsee.model.message.MessageDataset
import com.marked.pixsee.views.adapter.MessageAdapter
 ...
 * on handsets.
 */
class ChatDetailFragment : Fragment(), GCMListenerService.Callbacks {
	...
	private var mCallback: ChatDetailFragment.ContactDetailFragmentInteraction? = null
	...
	fun sendMessage(messageText: String, messageType: Int) {
		...
		mSocket.emit(ChatDetailFragment.Static.ON_NEW_MESSAGE, jsonObject)
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		mSocket.on(ChatDetailFragment.Static.ON_NEW_MESSAGE, onMessage)
		mSocket.on(ChatDetailFragment.Static.ON_TYPING, onTyping)

		mSocket.connect()

		mSocket.emit(ChatDetailFragment.Static.ON_NEW_ROOM, JSONObject("{from:%s,to:%s}".format(mThisUser, mThatUser.userID)))

	}

	override fun onDestroy() {
		super.onDestroy()
		mSocket.off(ChatDetailFragment.Static.ON_NEW_MESSAGE, onMessage)
		mSocket.off(ChatDetailFragment.Static.ON_TYPING, onTyping)
		mSocket.disconnect()
	}
	...
	override fun onPause() {
		super.onPause()
		ChatDetailFragment.Static.isInForeground = false
		mMessagesInstance.clear()
	}

	override fun onResume() {
		super.onResume()
		ChatDetailFragment.Static.isInForeground = true
	...
	override fun onAttach(context: Context?) {
		...
		try {
			mCallback = context as ChatDetailFragment.ContactDetailFragmentInteraction?
		} catch (e: ClassCastException) {
			throw ClassCastException(context!!.toString() + " must implement " +
					ChatDetailFragment.ContactDetailFragmentInteraction::class.java)
		}

	}

	fun onTyping(typing: Boolean) {
		mSocket.emit(ChatDetailFragment.Static.ON_TYPING, JSONObject("{from:%s,to:%s,typing:%s}".format(mThisUser, mThatUser.userID, typing)))
	}
	...
	companion object Static {
		...
		@JvmStatic
		fun newInstance(parcelable: Parcelable): ChatDetailFragment {
			val bundle = Bundle()
