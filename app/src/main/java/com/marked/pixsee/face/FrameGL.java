...
public class FrameGL {

	private static final String LOGTAG = "FrameGL";

	Selfie                 mActivity;
	ApplicationSession     vuforiaAppSession;
	// Projection and Modelview Matrices
	com.threed.jpct.Matrix projectionOrtho, modelview;
	// Color vector
	Vec4F color;
	...
	public FrameGL(Selfie activity, ApplicationSession session) {
		mActivity = activity;
	...
	boolean init(int screenWidth, int screenHeight) {
		...
		if ((shaderProgramID = SampleUtils.createProgramFromShaderSrc(frameVertexShader, frameFragmentShader)) == 0)
			return false;
		...
		// makes ortho matrix
		projectionOrtho = new com.threed.jpct.Matrix();
//		projectionOrtho = new Matrix44F();
	...
	void getTextures() {
		for (int i = 0; i < /*TEXTURE_NAME.TEXTURE_COUNT*/textureNames.length; i++)
			textures[i] = createTexture(textureNames[i]);
	}
	// Creates a texture given the filename
	private Texture createTexture(String nName) {
		return Texture.loadTextureFromApk(nName, mActivity.getAssets());
	}
	...
	void renderViewfinder() {
		...
		Matrix.multiplyMM(mvp, 0, projectionOrtho.getDump(), 0, modelview.getDump(), 0);
