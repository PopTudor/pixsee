...
class LogInRegistrationIntentService : IntentService("RegIntentService") {
    ...
    override fun onHandleIntent(intent: Intent) {
        try {
            ...
            when (intent.action) {
                LogInRegistrationIntentService.Companion.ACTION_LOGIN -> {
                    val param1 = intent.getStringExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_EMAIL)
                    val param2 = intent.getStringExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_PASSWORD)
                    handleActionLogin(param1, param2, token)
                }
                LogInRegistrationIntentService.Companion.ACTION_SIGNUP -> {
                    val param1 = intent.getStringExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_NAME)
                    val param2 = intent.getStringExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_EMAIL)
                    val param3 = intent.getStringExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_PASSWORD)
                    handleActionSignup(param1, param2, param3, token)
                }
                LogInRegistrationIntentService.Companion.ACTION_RECOVERY -> {
                    val param1 = intent.getStringExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_EMAIL)
    ...
    private fun subscribeTopics(token: String) {
        ...
        for (topic in LogInRegistrationIntentService.Companion.TOPICS) {
    ...
    companion object {
        ...
        fun startActionSignup(context: Context, name: String?, email: String?, password: String?) {
            ...
            intent.action = LogInRegistrationIntentService.Companion.ACTION_SIGNUP
            intent.putExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_NAME, name)
            intent.putExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_EMAIL, email)
            intent.putExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_PASSWORD, password)
        ...
        fun startActionLogin(context: Context, email: String, password: String) {
            val intent = Intent(context, LogInRegistrationIntentService::class.java)
            intent.action = LogInRegistrationIntentService.Companion.ACTION_LOGIN
            intent.putExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_EMAIL, email)
            intent.putExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_PASSWORD, password)
            context.startService(intent)
        }
        ...
        fun startActionRecovery(context: Context, email: String) {
            val intent = Intent(context, LogInRegistrationIntentService::class.java)
            intent.action = LogInRegistrationIntentService.Companion.ACTION_RECOVERY
            intent.putExtra(LogInRegistrationIntentService.Companion.EXTRA_PARAM_EMAIL, email)
            context.startService(intent)
        }
    }
