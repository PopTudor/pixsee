...
// ----------------------------------------------------------------------------------------
    ...
    const matrix<double, 1, 3> cross_validate_object_detection_trainer(
        ...
        for (long iter = 0; iter < folds; ++iter) {
            ...
            typename trainer_type::trained_function_type detector = trainer.train(array_subset, training_rects, training_ignores,
                                                                                  overlap_tester);
            for (unsigned long i = 0; i < test_idx_set.size(); ++i) {
                std::vector<std::pair<double, rectangle> > hits;
                detector(images[test_idx_set[i]], hits, adjust_threshold);

                correct_hits += impl::number_of_truth_hits(truth_dets[test_idx_set[i]], ignore[i], hits, overlap_tester, all_dets,
                                                           missing_detections);
